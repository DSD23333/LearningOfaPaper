...
class AttModel(Module):
    ...
    def __init__(self, in_features=48, kernel_size=5, d_model=512, num_stage=2, dct_n=20):
        ...
        self.linear_k = nn.Linear(48*10, d_model)
        self.linear_k1 = nn.Linear(48 * 10, d_model)
        self.linear_k2 = nn.Linear(48 * 10, d_model)
        self.linear_k3 = nn.Linear(48 * 10, d_model)

        self.linear_q = nn.Linear(48*10, d_model)
        self.linear_q1 = nn.Linear(48*10, d_model)
        self.linear_q2 = nn.Linear(48*10, d_model)
        self.linear_q3 = nn.Linear(48*10, d_model)
    ...
    def forward(self, src,
        ...
        # print(src_query_tmp.shape)
        src_query_tmp = torch.matmul(dct_kq[:dct_n].unsqueeze(dim=0), src_query_tmp).transpose(1, 2)
        # print(src_query_tmp.shape)
        ...
        # print(src_key_tmp.shape)
        src_key_tmp = torch.matmul(dct_kq[:dct_n].unsqueeze(dim=0), src_key_tmp)
