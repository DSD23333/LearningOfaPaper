...
def main(opt):
    ...
    if not opt.is_eval:
        ...
        #===train 数据集
        dataset = datasets.Datasets(opt, split=0) # 进行读取 并 打印
        print('>>> Training dataset length: {:d}'.format(dataset.__len__()))  # 打印样本总量
        # loader 进行样本打乱 打包batch numwork 数据加载的并行数 pin是固定内存来着（涉及加载的效率好像）
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (ang_h36) in enumerate(data_loader):
        ...
        ang_out_all = net_pred(ang_src,dct_n=opt.dct_n, output_n=out_n, itera=itera, input_n=in_n)
